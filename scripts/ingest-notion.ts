...
- NOTION_DB_ID
Optional:
- NOTION_TOKEN_PROP (name of a rich_text or title prop to write token back, e.g., 'Collection Token')
...
async function writeTokenToNotion(pageId: string, token: string, props: any) {
  try {
    if (!props || !(TOKEN_PROP in props)) return; // skip silently if property missing
    const link = `${BASE_URL}/id/${token}`;
    const p = props[TOKEN_PROP];
    const patch: any = {};
    if (p?.type === 'url') {
      patch[TOKEN_PROP] = { url: link };
    } else {
      patch[TOKEN_PROP] = { rich_text: [{ type: 'text', text: { content: token, link: { url: link } } }] };
    }
...
async function uploadImageFromUrl(db: ReturnType<typeof supabaseAdmin>, objectId: string, url: string) {
  try {
    ...
    const fileName = `${objectId}-${Date.now()}.${extFromType}`;
...
async function run() {
  ...
  for (const page of slice) {
    ...
    const existingToken = propText(props[TOKEN_PROP]);
    ...
    if (!existingToken) await writeTokenToNotion(id, token, props);
