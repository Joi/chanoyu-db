...
--   - 20250818151956_remote_schema.sql
--   - 20250821_local_classes.sql
--   - 20250903010000_add_local_classes_swap_function.sql
--   - 20250903020000_fix_local_classes_admin_policies.sql
--   - 20250903030000_local_classes_order.sql
--   - 20250903040000_create_chakai_table.sql
--   - 20250903050000_media_visibility_controls.sql
--   - 20250904_chakai_media_links.sql
--   - 20250905_chakai_items.sql
--   - 20250906_security_fixes.sql
...
-- RLS Policies (with proper idempotency checks)
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'local_classes' and policyname = 'local_classes_public_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'local_classes' and policyname = 'local_classes_admin_all'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'local_class_hierarchy' and policyname = 'local_class_hierarchy_public_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'local_class_links' and policyname = 'local_class_links_public_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'chakai_items' and policyname = 'chakai_items_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'chakai_items' and policyname = 'chakai_items_admin_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'chakai_items' and policyname = 'chakai_items_admin_write'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'chakai_media_links' and policyname = 'chakai_media_links_read'
    ) then
...
do $$ begin
    if not exists (
        select 1 from pg_policies
        where schemaname = 'public' and tablename = 'media' and policyname = 'media_public_read_chakai'
    ) then
...
-- Backfill existing data consistency
-- Update existing media records to have appropriate visibility defaults
...
set sort_order = coalesce(
    nullif(
        case
            when local_number ~ '^[0-9]+$' then (local_number)::int
            else null
        end,
        null
    ),
