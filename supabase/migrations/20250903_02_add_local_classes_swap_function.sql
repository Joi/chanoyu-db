...
CREATE OR REPLACE FUNCTION swap_local_class_sort_order(
  class_id_1 UUID,
  class_id_2 UUID
) RETURNS TABLE (
  success BOOLEAN,
...
COMMENT ON FUNCTION swap_local_class_sort_order IS 
'Atomically swaps sort_order between two top-level local_classes. Used for reordering functionality in admin interface.';