...
async function addImage(formData: FormData) {
  ...
  revalidatePath('/admin/items-and-images');
}
...
export default async function ItemsAndImagesPage() {
  ...
  const { data: objs, error: eObjs, count } = await db
    ...
    .limit(200);
  if (eObjs) console.error('[admin/items-and-images] query error', eObjs.message || eObjs);
  ...
  if (ids.length) {
    const { data: mediaRows, error: eMed } = await db
      ...
      .in('object_id', ids);
    if (eMed) console.error('[admin/items-and-images] media query error', eMed.message || eMed);
  ...
  return (
    <main className="max-w-5xl mx-auto p-6">
      <h1 className="text-xl font-semibold mb-4">Items and Images</h1>
      {!objectList.length ? (
        ...
        {objectList.map((o: any) => {
          ...
          return (
            <div key={o.id} className="card" style={{ display: 'grid', gridTemplateColumns: '140px 1fr', gap: 12 }}>
              <div>
                <div style={{ position: 'relative', width: 120, paddingTop: '66%', background: '#f5f5f5', border: '1px solid #eee', borderRadius: 6, overflow: 'hidden' }}>
                  {thumb ? (
                    <a href={`/media/${primaryMedia.id}`} title="Open media">
                      <Image src={thumb} alt={o.title} fill sizes="120px" style={{ objectFit: 'cover' }} />
                    </a>
                  ) : null}
                </div>
                  ...
                  {mediaSorted.slice(0, 6).map((m: any) => (
                    <div key={m.id} className="card">
                      <div style={{ position: 'relative', width: '100%', paddingTop: '66%', background: '#f5f5f5', border: '1px solid #eee', borderRadius: 6, overflow: 'hidden' }}>
                        {m.uri ? (
                          <a href={`/media/${m.id}`} title="Open media">
                            <Image src={m.uri} alt={o.title} fill sizes="160px" style={{ objectFit: 'cover' }} />
                          </a>
                        ) : null}
                      </div>
