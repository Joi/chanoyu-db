...
async function createAccount(formData: FormData) {
  ...
  if (!isOwner) role = 'guest'; // admins can only create guests
  if (role === 'owner' && !isOwner) return; // safety
  const password_hash = hashPassword(password);
  const db = supabaseAdmin();
  await db.from('accounts').insert({ email, full_name_en, full_name_ja, role, password_hash });
...
async function deleteAccount(formData: FormData) {
  ...
  if (!id) return;
  if (!isOwner && role !== 'guest') return; // admins cannot delete owner/admin
  const db = supabaseAdmin();
...
async function updateAccount(formData: FormData) {
  ...
  const password = String(formData.get('password') || '');
  if (!id) return;
  // Admins can only edit guests and cannot escalate role beyond guest
  if (!isOwner) {
...
export default async function AccountsPage() {
  ...
  const isOwner = await requireOwner();
  const db = supabaseAdmin();
  // Admins should not see owner/admin accounts to edit; list all but disable as needed
  const { data } = await db
    .from('accounts')
  ...
  return (
    <main className="max-w-3xl mx-auto p-6">
      ...
      <section className="card mb-6">
        ...
        <form action={createAccount} className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 12 }}>
        ...
        {!isOwner ? <p className="text-xs text-gray-600 mt-2">As admin, you can create guests only.</p> : null}
      </section>
        ...
        {(data ?? []).map((u: any) => {
          ...
          return (
            <div key={u.id} className="card">
              <form action={updateAccount} className="grid" style={{ gridTemplateColumns: '1fr 1fr', gap: 8 }}>
