...
CREATE OR REPLACE FUNCTION "public"."set_local_class_local_number"("class_id" integer, "new_local_number" "text") RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
begin
...
CREATE OR REPLACE FUNCTION "public"."swap_local_class_sort_order"("item1_id" integer, "item2_id" integer) RETURNS "void"
    LANGUAGE "plpgsql" SECURITY DEFINER
    SET "search_path" TO ''
    AS $$
declare
...
CREATE OR REPLACE VIEW "public"."local_class_object_counts_direct" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."local_class_object_counts_total" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE VIEW "public"."object_preferred_classification" WITH ("security_invoker"='true') AS
...
CREATE OR REPLACE TRIGGER "lc_after_insert_trigger" AFTER INSERT ON "public"."local_classes" FOR EACH ROW EXECUTE FUNCTION "public"."lc_after_insert"();



CREATE OR REPLACE TRIGGER "lc_before_update_parent_trigger" BEFORE UPDATE ON "public"."local_classes" FOR EACH ROW EXECUTE FUNCTION "public"."lc_before_update_parent"();
...
CREATE POLICY "chakai_items_read_optimized" ON "public"."chakai_items" FOR SELECT USING ((((("current_setting"('request.jwt.claims'::"text", true))::json ->> 'role'::"text") = 'admin'::"text") OR ("current_setting"('request.jwt.claim.role'::"text", true) = ANY (ARRAY['admin'::"text", 'owner'::"text"])) OR (EXISTS ( SELECT 1
   FROM "public"."chakai" "c"
  WHERE (("c"."id" = "chakai_items"."chakai_id") AND (("c"."visibility" = 'open'::"text") OR (("c"."visibility" = 'members'::"text") AND (("current_setting"('request.jwt.claim.role'::"text", true) = ANY (ARRAY['admin'::"text", 'owner'::"text"])) OR (EXISTS ( SELECT 1
          ...
          WHERE (("ca"."chakai_id" = "c"."id") AND (("a"."email" = "current_setting"('request.jwt.claim.sub'::"text", true)) OR ("a"."email" = (("current_setting"('request.jwt.claims'::"text", true))::json ->> 'email'::"text")))))))) OR (("c"."visibility" = 'closed'::"text") AND ("current_setting"('request.jwt.claim.role'::"text", true) = ANY (ARRAY['admin'::"text", 'owner'::"text"])))))))));
...
CREATE POLICY "local_classes_access_optimized" ON "public"."local_classes" USING (true);



ALTER TABLE "public"."local_counters" ENABLE ROW LEVEL SECURITY;


ALTER TABLE "public"."location_media_links" ENABLE ROW LEVEL SECURITY;
