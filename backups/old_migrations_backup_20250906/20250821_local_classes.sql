...
-- Core table: local_classes (without FK constraints first)
...
create unique index if not exists local_classes_local_number_ci on local_classes (lower(local_number))
where local_number is not null;
...
create or replace function set_local_class_local_number() returns trigger as $$ begin if new.local_number is null then new.local_number := 'ITO-C-' || lpad(nextval('local_class_local_seq')::text, 5, '0');
end if;
return new;
end;
$$ language plpgsql;
...
) then create trigger trg_local_class_local_number before
...
create or replace function lc_after_insert() returns trigger as $$ begin -- self link
insert into local_class_hierarchy (ancestor_id, descendant_id, depth)
values (new.id, new.id, 0) on conflict do nothing;
if new.parent_id is not null then -- link all ancestors of parent (including parent) to the new node
insert into local_class_hierarchy (ancestor_id, descendant_id, depth)
select a.ancestor_id,
    new.id,
    a.depth + 1
from local_class_hierarchy a
where a.descendant_id = new.parent_id on conflict do nothing;
end if;
return null;
end;
$$ language plpgsql;
...
do $$ begin if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_lc_after_insert'
) then create trigger trg_lc_after_insert
after
...
begin if new.parent_id is distinct
from old.parent_id then if new.parent_id is not null then if new.parent_id = new.id then raise exception 'local_classes: parent_id cannot equal id';
end if;
...
if has_cycle then raise exception 'local_classes: parent_id cannot create a cycle';
end if;
end if;
-- Rewire closure for the subtree rooted at NEW.id
-- 1) Delete links from former ancestors to the subtree
delete from local_class_hierarchy h using (
...
where a.descendant_id = new.parent_id
    and d.ancestor_id = new.id on conflict do nothing;
end if;
end if;
return new;
end;
...
do $$ begin if not exists (
    select 1
    from pg_trigger
    where tgname = 'trg_lc_before_update_parent'
) then create trigger trg_lc_before_update_parent before
update of parent_id on local_classes for each row execute function lc_before_update_parent();
end if;
end $$;
...
do $$ begin if not exists (
    ...
    where table_schema = 'public'
        ...
        and column_name = 'primary_local_class_id'
) then
alter table objects
add column primary_local_class_id uuid references local_classes(id);
end if;
exception
...
do $$ begin if not exists (
        ...
        where schemaname = 'public'
            ...
            and policyname = 'local_classes_public_read'
    ) then create policy local_classes_public_read on local_classes for
select using (true);
end if;
end $$;
...
do $$ begin if not exists (
        ...
        from pg_policies
        where schemaname = 'public'
            and tablename = 'local_class_links'
            and policyname = 'local_class_links_public_read'
    ) then create policy local_class_links_public_read on local_class_links for
select using (true);
end if;
end $$;
...
do $$ begin if not exists (
        ...
        from pg_policies
        where schemaname = 'public'
            and tablename = 'local_class_hierarchy'
            and policyname = 'local_class_hierarchy_public_read'
    ) then create policy local_class_hierarchy_public_read on local_class_hierarchy for
select using (true);
end if;
end $$;
...
do $$ begin 
    if not exists (
        ...
        alter table local_classes 
        add constraint local_classes_parent_id_fkey 
        foreign key (parent_id) references local_classes(id) on delete set null;
    end if;
end $$;
...
do $$ begin 
    if not exists (
        ...
        alter table local_classes 
        add constraint local_classes_preferred_classification_id_fkey 
